...
**Status bar** (baked-in, someblocks-style: `icon ++ first stdout line`, re-run per interval and/or on `SIGRTMIN+n`). Blocks: `ip` (30s), `audio` (60s, RTMIN+1), `volume` (1s, RTMIN+1), `mic` (1s, RTMIN+2), `date` (1s), `battery` (30s). Scripts in `de/.config/reach/blocks/`. **Blocks can colour their own output:** reach parses dwlb-style inline markup in a block's stdout — `^fg(RRGGBB)`…`^fg()` (bare `^fg()` resets to `status_fg`, which is only the *default*, not a per-area limit). `mic.sh` uses it to flag an unmuted mic red, and `battery.sh` wraps the whole block in `^fg(f38ba8)` under 20% while discharging. (This is not discoverable from `config.zon` or from the binary's string table — read the block scripts.) `battery.sh` also picks its glyph by level (fa `battery_4`…`battery_0`). Keybinds poke it with `kill -35 $(pidof reach)` (audio/volume) and `kill -36` (mic) for instant refresh.

**Keybinds:** `Super`=Mod. A `.binds` block **replaces the entire** default action/spawn/chord keymap **except** the auto-generated per-tag digit binds (`Super`/`+Ctrl`/`+Shift` + `1`–`9` = view/toggle/move, never listed in config). See `config.zon` for the full map. Notable: launchers (`Super+Tab` kitty, `Super+Space` quickshell launcher (`qs ipc call launcher toggle`, replaced rofi), `Super+Shift+Escape` shell settings (`qs ipc call settings toggle`), `Super+V` clipfzf, `Super+X` killfzf, `Super+Z` svfzf), `Super+r`/`Super+s` two-key chords (apps — incl. `a` = audio mixer / screenshots), `Alt+[` cycle EQ sink, `Super+Alt+Left/Right` brightness, `Super+Alt+Up` night-light toggle, `Super+P` lock (`qs ipc call lock lock`; the lock screen also carries poweroff/reboot/logout), `Super+b` random wallpaper (`qs ipc call wallpaper random`), `Super+c` week calendar (`qs ipc call calendar toggle`).
...
- `CalendarWeek.qml` — Outlook-style week grid, a full-screen **overlay** (`Super+c` / `qs ipc call calendar toggle`; also `next`/`prev`/`today`/`hide`). Separate from `Calendar.qml` rather than replacing it: the ambient card is pinned to a 420x451 slot, and seven day columns in 420px leaves no room for titles — a grid only reads at overlay size. Fed by `calendar.sh week [offset]`, which emits *geometry* (day index + minutes from midnight) rather than the pre-formatted strings `events` returns. Non-obvious bits: (1) the grid is always the **full 24h day** — sizing it to the week's event range made `contentHeight` exactly equal the viewport, so nothing scrolled and any hour outside 8am-8pm was unreachable; it opens scrolled to the week's first event instead (deferred via `Qt.callLater`, since the Column has not sized when the JSON lands). (2) Overlapping events are packed into lanes per *cluster* of transitively-overlapping events, not per day, so a 9am collision doesn't shrink an unrelated 5pm block. (3) Event colours arrive from JSON as strings — they are assigned through a `color`-typed property before `.r/.g/.b` is read, or the tint comes out undefined. (4) Same per-output pointer-latching as `Launcher.qml`, but the no-pointer fallback resolves against `Quickshell.screens` instead of hardcoding `DP-2` (which draws nothing on the laptop). (5) `qs ipc call calendar show` does **not** work from the CLI — `show` collides with the `qs ipc show` subcommand and silently prints the function list; use `toggle`. Same quirk applies to `Launcher`.
- `HoverNote.qml` — hover tooltip for elided grid cells; named to avoid colliding with `QtQuick.Controls`' `ToolTip`.
